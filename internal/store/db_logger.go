...
func (d *loggedDB) ExecContext(
	...
	rowsAffected := safeRowsAffected(err, res)
...
// safeRowsAffected returns the rows-affected count if available, otherwise -1.
// Some drivers (notably go-sqlcipher) return a Result whose underlying
// driver.Result is nil for multi-statement DDL execs, which makes
// RowsAffected() panic. Recover so logging never crashes the caller.
func safeRowsAffected(err error, res sql.Result) (n int64) {
	n = -1
	if err != nil || res == nil {
		return
	}
	...
	if v, rerr := res.RowsAffected(); rerr == nil {
		n = v
	}
	return
}
