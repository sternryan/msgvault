...
type Options struct {
	...
	// CheckpointInterval is the number of messages between checkpoint
	// writes (default: 200).
...
func (s *Syncer) storeAttachment(messageID int64, att *mime.Attachment) error {
	...
	// Record in database
	return s.store.UpsertAttachment(messageID, att.Filename, att.ContentType, storagePath, att.ContentHash, att.ContentID, len(att.Content))
}
